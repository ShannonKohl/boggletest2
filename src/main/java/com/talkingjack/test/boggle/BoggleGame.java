...
public class BoggleGame extends JFrame {
     ...
     */
    public BoggleGame(int rows, int cols) throws FileNotFoundException {
        ...
        shakespeareDictionary = scan( "dictionary-shakespeare.txt" );

        // all words in nursery
        nurseryDictionary = scan( "dictionary-nursery.txt" );

        // about 20K common words
        commonDictionary = scan( "dictionary-common.txt" );

        // all words in Algorithms 4/e
        algs4Dictionary = scan( "dictionary-algs4.txt" );

        // dictionary
        Set<String> dictionarySet = scan( "dictionary-yawl.txt");

        // create the Boggle solver with the given dictionary
        solver = new BoogleSolver( dictionarySet.toArray( new String[ dictionarySet.size()]));

        newGame();
        this.pack();   
    }
    private Set<String> scan( String filename ) throws FileNotFoundException{
    ...
    public static void main(final String ... args) throws Throwable {
        //NOTICE do not need to run on Swing thread until setVisible is called, since that is last, we are ok to run this on the calling thread.
